...
from ML import rank_and_optimize as ml_rank_and_optimize
from ML import recommend_payload
from ML import update_preferences as ml_update_preferences
...
class MlRecommendationService:
    ...
    def rank_and_optimize(
        ...
        *,
        ...
        selected_by_id = {
            (offer.product_id, offer.variant_id): offer for offer in selected
        }
        return [
            offer.model_copy(
                update={
                    "match_score": selected_by_id[
                        (offer.product_id, offer.variant_id)
                    ].match_score
                }
            )
            for offer in offers
            if (offer.product_id, offer.variant_id) in selected_by_id
        ]
